...
import { setInputPasswordPageOn, setSignInEmail, setSignInPageOn, setSignUpPageOn } from "../../data/store";
...
const EmailInput = styled.input`
  ...
  border: 1px solid;
  /* errorMessageOn */
  border-color: var(--theme-palette-colors-bluegray-200);
  ...
  &:focus {
    border: 1px solid;
    border-color: ${(props) => (props.errorMessageOn ? "var(--theme-palette-colors-red-400)" : "var(--theme-palette-colors-blue-400)")};
  }
  ::placeholder {
    color: #ccc;
  }
...
const TermsBox = styled.div`
  ...
  a {
    color: var(--theme-palette-colors-gray-700);
    font-weight: var(--text-weight-footnote-bold);
    text-align: center;
...
const Inc = styled.p`
  color: var(--theme-palette-colors-gray-600);
  font-weight: var(--text-weight-caption1-normal);
  text-align: center;
  letter-spacing: var(--text-spacing-caption1);
  font-size: var(--text-size-caption1);
  line-height: var(--text-height-caption1);
  ...
  const emailCheck = (e) => {
    ...
    if (email !== "") {
      if (validateEmail(email)) {
        //이메일 형식확인
        setSubmitOn(true);
        setErrorMessageOn(false);
      } else {
        setSubmitOn(false);
        setErrorMessageOn(true);
      }
    } else {
      setSubmitOn(false);
      setErrorMessageOn(false);
    ...
    <LoginContentBox>
      ...
      <LoginForm>
        ...
        <EmailInput
          ...
          onFocus={(e) => emailCheck(e)}
          onChange={(e) => emailCheck(e)}
          placeholder="이메일을 입력해주세요"
        ></EmailInput>
        {errorMessageOn && <ErrorMessage>올바른 이메일을 입력해주세요</ErrorMessage>}
        <NextButton type="button" onClick={submit} disabled={!submitOn}>
          <BtnText className={!submitOn && "disabled"}>이메일로 계속하기</BtnText>
        </NextButton>
        <p className="or-p-tag">또는</p>
        <SocialLoginBox>
          <SocialLoginBtn>
            <span>
          ...
          <SocialLoginBtn className="google-svg">
            <span>
              <svg viewBox="0 0 57 56" className="css-1h47l4s">
                <path
        ...
        <ForgotAccount>
          <Link to={"/"}>
            계정을 잊으셨나요?
            <span>
              <svg viewBox="0 0 12 12" color="var(--theme-palette-colors-gray-600)" className="css-2zw3f5">
                <path
        ...
        <SectionDivider />
        <TermsBox>
          <Link to={"https://id.wanted.jobs/terms/ko"}>이용약관</Link>
          <Link to={"https://id.wanted.jobs/privacy/ko"}>개인정보처리방침</Link>
        </TermsBox>
        <Inc>© Wantedlab, Inc.</Inc>
        <LanguageSelectBox>
          <SelectBoxWrapper>
            <FlagImg src="https://static.wanted.co.kr/images/wantedoneid/ico_KR.svg" />
            <SelectBox>
              <select>
                <option defaultValue={"ko_KR"}>한국어</option>
