...
function MainEventSlide() {
  ...
  const [showNextArrow, setShowNextArrow] = useState(eventBannerData.length > 1);
      ...
      <SliderContainer>
        <EventBannerSlider ref={(slider) => (slider1.current = slider)} {...settings}>
          {eventBannerData.map((data, i) => {
            if (i < 6) {
              return <EventItem key={data.id} eventData={data} />;
            } else if (i === 6) {
              return <EventItem key={data.id} eventData={data} filterd={true} />;
            }
          })}
        </EventBannerSlider>
      </SliderContainer>
